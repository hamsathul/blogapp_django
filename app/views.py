...
def index(request):
	...
	if request.POST:
		...
		if subscribe_form.is_valid:
			...
			subscribe_sucessful = 'Subscribed Sucessfully'
			subscribe_form = SubscribeForm()

	context ={'posts':posts, 'top_posts':top_posts, 'website_info':website_info, 'recent_posts':recent_posts, 'subscribe_form':subscribe_form, 'subscribe_sucessful':subscribe_sucessful,'featured_blog':featured_blog}
		...
		if comment_form.is_valid:
			...
			if request.POST.get('parent'):
				...
				if parent_obj:
					...
					comment_reply.save()
					return HttpResponseRedirect(reverse('posts_page', kwargs={'slug':slug}))
...
def author_page(request, slug):
	...
	top_post = Post.objects.filter(author = author.user).order_by('-view_count')[0:2]
	recent_post = Post.objects.filter(author = author.user).order_by('-last_updated')[0:2]
	top_authors = User.objects.annotate(number=Count('post')).order_by('number')
...
def bookmark_post(request, slug):
	...
	return HttpResponseRedirect(reverse('posts_page', args=[str(slug)]))
